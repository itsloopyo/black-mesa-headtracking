@echo off
:: ============================================
:: Black Mesa Head Tracking - Install
:: ============================================
...
:: --- CONFIG BLOCK ---
set "GAME_ID=black-mesa"
set "MOD_DISPLAY_NAME=Black Mesa Head Tracking"
set "MOD_DLLS=BlackMesaHeadTracking.asi"
set "MOD_INTERNAL_NAME=BlackMesaHeadTracking"
...
:: their proxy DLL from somewhere other than beside the exe. EMPTY here, and it
:: has to stay empty: bin\ works on a direct launch and never under Steam,
:: because gameoverlayrenderer.dll is injected first and pulls WINMM.dll in from
:: System32. A base name already in the module list can no longer resolve to our
:: proxy, so nothing scans for the .asi and the mod is silently absent. winmm.dll
:: is not a KnownDLL, so the exe directory is searched ahead of System32 and the
:: root copy wins even that early load. Set the same value in uninstall.cmd.
set "ASI_SUBDIR="
