...
void WriteView(cameraunlock::IniWriter& w) {
    ...
    w.WriteComment(" Black Mesa registers no viewmodel_fov cvar, so there is no viewmodel");
    w.WriteComment(" counterpart to this key: widening Fov leaves the weapon drawn at the");
    w.WriteComment(" size the game chose, looking larger against the wider world.");
    w.WriteDouble("Fov", kDefaultFovOverride);
}

void WriteDebug(cameraunlock::IniWriter& w) {
    w.WriteSection("Debug");
    w.WriteComment(" Writes HeadTracking.log next to bms.exe, fresh every launch (the");
...
// One Win32 call rather than std::filesystem::exists. Even the error_code
// overload drags <filesystem> into a binary that needs nothing else from it,
// and this runs on the bootstrap thread before the log is open, where anything
// that can throw is a terminated game. Core asks the same question the same way
// when it rotates the log.
...
void WriteDefaultIni(const std::string& path) {
    ...
    w.WriteComment(" Black Mesa head tracking - default config");
...
bool Config::FileLoggingRequested() {
    // Open() is the existence check: it returns false when the file is not
    // there, which is the same answer for a config that has not been written
    // yet and one that cannot be read. Nothing is written here either way - a
    // missing ini is created by LoadOrCreateDefault, after the log is open.
...
Config Config::LoadOrCreateDefault() {
    ...
    // (and a failed write says so) rather than the mod running on built-in
