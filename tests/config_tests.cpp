...
// fresh render, the defaults v0.1.1 ran on map to the defaults, a first start creates the
...
// and a folder the ANSI code page cannot name imports as v0.1.1 read it.
//
...
std::string Committed() {
    return ReadBytes(fs::path(BMS_COMMITTED_CONFIG));
}
...
struct Scratch {
    ...
    explicit Scratch(const std::wstring& gameFolder) {
        ...
        root = fs::path(temp) / (L"bms-config-tests-" + std::to_wstring(GetCurrentProcessId()));
...
// A fresh install and an upgrade from v0.1.1's defaults start the same: the map of the frozen
...
void TestLegacyDefaultsMapToTheDefaults() {
    wchar_t temp[MAX_PATH];
    GetTempPathW(MAX_PATH, temp);
    const fs::path missing = fs::path(temp) / L"bms-no-such-folder" / kLegacyConfigFileName;
    ...
    Check(result.dropped.empty(), "v0.1.1's defaults drop nothing");
    ...
    Check(AllValues(mapped) == AllValues(table.defaults()), "v0.1.1's defaults map to the defaults");
...
// v0.1.1 took its folder from core's HostExeDirectoryNarrow, which refuses a folder the ANSI
...
// it was.
void TestAFolderTheCodepageCannotNameImportsAsV011ReadIt() {
    const Scratch s(L"bw-\x4E2D");
    const std::string legacyBytes = "[Network]\r\nPort=5000\r\n";
    WriteBytes(s.LegacyPath(), legacyBytes);
    const auto loaded = cfg::ConfigOwner<Config>(s.Options()).Load();
    if (GetACP() == CP_UTF8) {
    ...
    Check(loaded.status == cfg::ConfigLoadStatus::Migrated, "a file v0.1.1 never read migrates to its defaults");
    Check(loaded.config.udp_port == 4242, "the port is v0.1.1's default, as that build ran");
    Check(ReadBytes(s.LegacyPath()) == legacyBytes, "HeadTracking.ini stays as it was");
}

// [View] Fov takes 0 or 30 to 150, as v0.1.1 did. A value outside keeps the row's default, with
// a diagnostic naming the line.
void TestFovOutsideItsRangeIsRefused() {
    const auto table = MakeConfigTable();
    const auto read = [&table](const std::string& value) {
        Config c = table.defaults();
        const cfg::CanonicalIni doc = cfg::ParseCanonicalIni("[View]\r\nFov=" + value + "\r\n");
        const bool clean = cfg::ApplyCanonical(doc, table, c).diagnostics.empty();
        return std::make_pair(clean, c.fov_override);
    };
    for (const char* ok : {"0", "30", "90.5", "150"}) {
        const auto r = read(ok);
        Check(r.first && r.second == std::stof(ok), std::string("FOV ") + ok + " is read");
    }
    for (const char* refused : {"29.9", "1", "150.5", "-1", "180"}) {
        const auto r = read(refused);
        Check(!r.first && r.second == 0.0f, std::string("FOV ") + refused + " is refused and keeps 0");
    }
}
...
int main(int argc, char** argv) {
    try {
        ...
        TestTogglesSave();
        TestAFolderTheCodepageCannotNameImportsAsV011ReadIt();
