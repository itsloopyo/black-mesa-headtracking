...
// Camera maths matching the Source Engine's conventions. Our own code: no Valve
// source file or header is copied into this repository, included by it or
// compiled into the binary. These are the standard Euler-basis formulas, and an
// injected view has to compose exactly the way the renderer's does or it will
// not line up, so there is only one set of results that works. Each is pinned
// against the engine's observed behaviour by the characterization tests in
// tests/.
//
