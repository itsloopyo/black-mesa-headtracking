...
void TrackerFeed::Start(const Config& config) {
    ...
    if (m_receiver.Start(m_port)) {
        ...
        // Why the bind failed is on the [receiver] line above, from the OS.
        // Naming a cause here would be a guess, and "port busy" is the wrong
        // guess often enough to send the reader after the wrong tracker app.
        HT_LOG("[plugin] UDP %u not bound yet - the receiver retries in the background and "
               "tracking starts on its own once the port frees up", m_port);
    }
}
