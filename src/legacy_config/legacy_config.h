...
// The HeadTracking.ini reader of v0.1.1, the last build that read that file, frozen so a player
// updating from any older build is converted exactly as that build read it. v0.1.0 read it with
// the same code. Nothing in this folder is ever edited: tests/config_differential/ pins every
// file here by hash.
//
// Three things differ from the reader it was taken from (src/config.cpp at v0.1.1):
...
struct Config {
    ...
    // Metres. LimitY bounded both vertical directions, up and down.
    float pos_limit_x      = 0.30f;
    float pos_limit_y      = 0.20f;
    float pos_limit_z      = 0.40f;
    float pos_limit_z_back = 0.10f;
    // Source units per metre of head movement.
    float pos_world_scale  = 39.37f;
    ...
    // Degrees, as fov_desired. 0 leaves the game's own.
...
// Reads the file at `path`, the ANSI path v0.1.1 opened it by, into a default-constructed `c`.
