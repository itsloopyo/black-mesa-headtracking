...
DWORD WINAPI BootstrapThread(LPVOID) {
    try {
        ...
        if (exeDir.empty()) {
            ...
            HT_LOG("[main] BlackMesaHeadTracking %s: Windows reported no folder for bms.exe, so "
        ...
        // Last, because it sleeps until the game has a settled window: nothing
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD reason, LPVOID /*reserved*/) {
    switch (reason) {
        case DLL_PROCESS_ATTACH:
            ...
            // "BlackMesaHeadTracking.asi_unloaded", usually
