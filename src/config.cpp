...
ImportResult Import(const LegacyInput& input, Config& out) {
    // v0.1.1 opened the file by the ANSI path of bms.exe's folder. Where that path has a
    ...
    // up in the Windows folder, not beside bms.exe: it never read the player's file and ran on
    // its defaults.
    legacy::Config c;
    ...
    // value the player changed is dropped. WorldScale shipped at kWorldUnitsPerMetre, which the
    // lean now converts by as a constant.
